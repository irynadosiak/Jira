...
class TaskFilteringTests(TestCase):
    ...
    def test_filter_by_assignee(self):
        ...
        response = self.client.get(
            reverse("tasks:task-list"), {"assignee": str(str(self.user1.id))}
        )
        self.assertEqual(response.status_code, 200)
        tasks = response.context["tasks"]
    ...
    def test_filter_by_reporter(self):
        ...
        response = self.client.get(
            reverse("tasks:task-list"), {"reporter": str(self.user2.id)}
        )
        self.assertEqual(response.status_code, 200)
        tasks = response.context["tasks"]
    ...
    def test_filter_unassigned_tasks(self):
        ...
        response = self.client.get(
            reverse("tasks:task-list"), {"assignee": "unassigned"}
        )
        self.assertEqual(response.status_code, 200)
        tasks = response.context["tasks"]
        self.assertEqual(len(tasks), 1)
    ...
    def test_combined_filters(self):
        ...
        response = self.client.get(
            ...
            {"status": "done", "assignee": str(str(self.user1.id)), "priority": "low"},
        )
        self.assertEqual(response.status_code, 200)
        tasks = response.context["tasks"]
        self.assertEqual(len(tasks), 1)
    ...
    def test_filter_with_search(self):
        ...
        response = self.client.get(
            reverse("tasks:task-list"), {"status": "todo", "search": "todo"}
        )
        self.assertEqual(response.status_code, 200)
        tasks = response.context["tasks"]
        self.assertEqual(len(tasks), 1)
    ...
    def test_invalid_filter_values(self):
        ...
        response = self.client.get(
            reverse("tasks:task-list"),
            {"assignee": "99999", "status": "invalid_status"},  # Non-existent user ID
        )
        self.assertEqual(response.status_code, 200)
        tasks = response.context["tasks"]
...
class APIFilteringTests(TestCase):
    ...
    def test_api_filter_by_status(self):
        ...
        response = self.client.get(
            reverse("tasks:api-task-list-create"), {"status": "todo"}
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_api_filter_by_assignee(self):
        ...
        response = self.client.get(
            reverse("tasks:api-task-list-create"), {"assignee": str(str(self.user1.id))}
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["count"], 1)
        self.assertEqual(data["results"][0]["assignee"]["id"], str(self.user1.id))

    def test_api_search(self):
        """Test API search functionality."""
        response = self.client.get(
            reverse("tasks:api-task-list-create"), {"search": "API Task 1"}
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["count"], 1)
    ...
    def test_api_ordering(self):
        ...
        response = self.client.get(
            reverse("tasks:api-task-list-create"), {"ordering": "title"}
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
    ...
    def test_api_multiple_filters(self):
        ...
        response = self.client.get(
            reverse("tasks:api-task-list-create"),
            {"status": "todo", "priority": "high", "assignee": str(self.user1.id)},
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["count"], 1)
        ...
        self.assertEqual(result["assignee"]["id"], str(self.user1.id))
