...
def dashboard(request):
    ...
    for i in range(7):
        ...
        completed_count = Task.objects.filter(
            status="done", updated_at__date=date
        ).count()
        completion_trend.append(
            {"date": date.strftime("%m/%d"), "completed": completed_count}
        )

    # Simple completion time calculation
    completed_tasks_with_due_date = Task.objects.filter(
        status="done", due_date__isnull=False
    )
    completion_times = []
    ...
    # Recent tasks
    recent_tasks = Task.objects.select_related("assignee", "reporter").order_by(
        "-updated_at"
    )[:6]
