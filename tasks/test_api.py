...
class TaskAPITests(APITestCase):
    ...
    def test_create_task_with_invalid_assignee(self):
        ...
        url = reverse("tasks:api-task-list-create")
        data = {"title": "Task with invalid assignee", "assignee_id": 99999}
        response = self.client.post(url, data)
    ...
    def test_create_task_with_invalid_status(self):
        ...
        url = reverse("tasks:api-task-list-create")
        data = {"title": "Task with invalid status", "status": "invalid_status"}
        response = self.client.post(url, data)
    ...
    def test_task_serialization_completeness(self):
        ...
        for field in expected_fields:
            self.assertIn(
                field, response.data, f"Field '{field}' missing from serialization"
            )
    ...
    def test_activity_creation_on_task_changes(self):
        ...
        self.assertTrue(
            any("Assignee changed" in desc for desc in activity_descriptions)
        )
        self.assertTrue(
            any("Priority changed" in desc for desc in activity_descriptions)
        )
